...
impl QuoteServer {
    ...
    fn handle_client(mut stream: TcpStream) -> Result<(), Error> {
        ...
        let quote = QUOTES[thread_rng().gen_range(0..QUOTES.len())];
        stream.write(quote.as_bytes())?;
        Ok(())
    }
}
...
impl QuoteClient {
    fn connect(&mut self) {
        ...
        println!("Quote of the day: {}", String::from_utf8_lossy(&buffer[..]));
    }
}

fn main() {
    let mut quote_client = QuoteClient {
        ip: String::from("127.0.0.1"),
        port: 6017,
    };
    let quote_server = QuoteServer {
        ip: String::from("127.0.0.1"),
        port: 6017,
    };
    quote_server.listen().unwrap();
